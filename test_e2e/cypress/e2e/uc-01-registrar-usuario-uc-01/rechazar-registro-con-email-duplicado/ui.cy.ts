...
describe("Rechazar registro con email duplicado", () => {
  it("Rechazar registro con email duplicado", () => {
    ...
    cy.get("[data-cy=\"error-message\"]", { timeout: 10000 }).should('be.visible'); cy.contains("El usuario o email ya existe", { matchCase: false, timeout: 10000 }).should('be.visible'); cy.contains("Usuario registrado exitosamente", { matchCase: false, timeout: 10000 }).should('be.visible');
  });
});
