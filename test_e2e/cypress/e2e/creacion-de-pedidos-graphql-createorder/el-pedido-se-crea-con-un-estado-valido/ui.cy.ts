 ...
 * Type: UI
 * Evidence summary: pages=/register selectors=#acceptTerms, #confirmPassword, #email, #errorMessage, #newsletter, #password, #sendMessage, #successMessage, #username, [name="role"] messages=Error al actualizar estado del pedido, Pedido creado exitosamente, Pedido cancelado exitosamente
 */

describe("El pedido se crea con un estado válido", () => {
  it("El pedido se crea con un estado válido", () => {
    ...
    const runScenario = (user: { username: string; email: string; password: string }) => {
      ...
      cy.location('pathname', { timeout: 10000 }).should('include', "/products"); cy.get("#successMessage", { timeout: 10000 }).should('be.visible'); cy.contains("Error al actualizar estado del pedido", { matchCase: false, timeout: 10000 }).should('be.visible');
    };
