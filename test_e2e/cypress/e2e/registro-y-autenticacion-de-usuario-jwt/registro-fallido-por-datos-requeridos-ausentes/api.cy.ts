 ...
 * Type: API
 * Evidence summary: endpoints=/api/auth/login, /api/auth/register messages=Error al obtener usuarios, ❌ Error de conexión. Intenta nuevamente., Rol inválido
 */
  ...
  it("Registro fallido por datos requeridos ausentes", () => {
    ...
    payload["email"] = 'invalid-value';
      cy.request({ method: "POST", url: "/api/auth/login", body: payload, failOnStatusCode: false }).then((res) => {
        ...
        expect(bodyText).to.contain("Error al obtener usuarios");
      });
    });
  });
});
