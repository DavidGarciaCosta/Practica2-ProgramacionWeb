 ...
 * Type: UI
 * Evidence summary: pages=/register selectors=#acceptTerms, #confirmPassword, #email, #errorMessage, #newsletter, #password, #successMessage, #username, [name="role"] messages=Error al obtener usuarios, ❌ Error de conexión. Intenta nuevamente., Rol inválido
 */

describe("Registro fallido por datos requeridos ausentes", () => {
  it("Registro fallido por datos requeridos ausentes", () => {
    ...
    const runScenario = (user: { username: string; email: string; password: string }) => {
      ...
      cy.location('pathname', { timeout: 10000 }).should('include', "/products"); cy.get("#successMessage", { timeout: 10000 }).should('be.visible'); cy.contains("Error al obtener usuarios", { matchCase: false, timeout: 10000 }).should('be.visible');
    };
